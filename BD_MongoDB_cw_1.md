...
```js
db.student.updateOne({ student_id: 1 }, { $set: { "grades.2": 12 } });
```
...
db.employees.find({
  $and: [{ "Address.Country": "USA" }, { TitleOfCourtesy: "Ms." }],
});
...
db.employees
  .find({ $and: [{ "Address.Country": "USA" }, { TitleOfCourtesy: "Ms." }] })
  .count();
...
db.employees.find({
  $and: [{ Salary: { $gt: 1100 } }, { Salary: { $lt: 1500 } }],
});
```
...
```js
db.employees.find({
  $or: [{ Salary: { $lte: 1100 } }, { Salary: { $gte: 1500 } }],
});
```
